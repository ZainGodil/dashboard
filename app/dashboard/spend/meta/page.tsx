...
export default async function MetaSpendPage({ searchParams }: PageProps) {
  ...
  const [{ data }, { data: overridesData }] = await Promise.all([
    ...
    supabase.from('campaign_overrides').select('campaign_name, course, university').eq('platform', 'meta'),
  ])
  const rows = data ?? []
  ...
  const tableRows = Array.from(aggMap.entries())
    .map(([key, d]) => {
      ...
      return { university: uni, course, ...d, cpm: d.impressions > 0 ? (d.spend / d.impressions) * 1000 : 0 }
    })
  ...
  return (
    <div>
      <header className="h-[60px] bg-white border-b border-slate-200 flex items-center px-6 gap-3 sticky top-0 z-50 shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
        <h1 className="font-display text-[15px] font-bold text-slate-900 tracking-tight">Meta Spend</h1>
      ...
      <div className="p-6 space-y-4">
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="bg-slate-50 text-slate-500 text-[11px] uppercase tracking-wider">
                  ...
                  <th className="text-right px-4 py-2.5 font-semibold">CPM</th>
                </tr>
              </thead>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          ...
          <CampaignsTable campaigns={allCampaigns} totalSpend={totalSpend} platform="meta" />
        </div>
      </div>
    </div>
  )
